...
function updateBot(room) {
  ...
  let speed = brain.mode === "ambush" ? BOT_MAX_SPEED : brain.mode === "bait" ? 2550 : 3300;
  ...
  const threats = room.state.pucks
    ...
    .map((puck) => {
      ...
      const predictedGuardX = predictPuckXAtY(puck, defensiveY);
  ...
  if (threats.length > 0) {
    ...
    if (mustGuard) {
      ...
      speed = clamp(3600 + puckSpeed * 0.78, 3800, BOT_MAX_SPEED);
    } else {
